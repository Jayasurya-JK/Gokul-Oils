...
export default function ProductInfo({ product, variations = [] }: ProductInfoProps) {
    ...
    // Use default attributes to find the initial selected variation, otherwise default to the first one
    const [selectedVarId, setSelectedVarId] = useState<number>(() => {
        ...
        if (product.default_attributes && product.default_attributes.length > 0) {
            const defaultVar = variations.find(v =>
                product.default_attributes.every(da => {
                    const vAttr = v.attributes.find(va => va.name === da.name);
                    return vAttr && vAttr.option === da.option;
                })
            );
    ...
    const getVariantLabel = (v: WooVariation) => {
        ...
        return attr ? attr.option : (v as any).name || 'Option';
    };

    const handleAddToCart = () => {
        // Use generic image property or first image from array, depending on object type
        let imageUrl = '';
        if ('image' in activeData && activeData.image) {
            imageUrl = (activeData as any).image.src;
        } else if (activeData.images && activeData.images.length > 0) {
            imageUrl = activeData.images[0].src;
        ...
        addToCart({
            id: activeData.id,
            name: product.name, // Keep parent name usually better for cart? Or activeData.name? Variation name might be just "Blue", we want "T-Shirt - Blue"
            price: price,
            quantity: quantity,
            image: imageUrl,
            slug: product.slug,
    ...
    if (currentVariation && 'image' in currentVariation && (currentVariation as any).image) {
        const varImage = (currentVariation as any).image as WooProductImage;
        const otherVarImages = variations
            .filter(v => v.id !== currentVariation.id && (v as any).image)
            .map(v => (v as any).image.src);

        if (varImage && varImage.src) {
            // Start with the specific variation image
            // Then include parent images, BUT exclude images that belong to OTHER variations
            // This keeps "common" images (like nutrition facts) while hiding the 1L bottle when 5L is selected
            displayImages = [
                varImage,
                ...product.images.filter(img => img.src !== varImage.src && !otherVarImages.includes(img.src))
            ];
        }
    }
            ...
            <div className="flex flex-col">
                ...
                <div className="flex items-center gap-2 mb-4">
                    <div className="flex text-yellow-400">
                        {[1, 2, 3, 4, 5].map((star) => (
                            <Star key={star} className={`w-3.5 h-3.5 ${star <= Math.round(parseFloat(product.average_rating || "0")) ? 'fill-current' : 'text-gray-300'}`} />
                        ))}
                    </div>
                    <span className="text-xs text-gray-500">({product.rating_count || 0} reviews)</span>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 py-4 border-t border-gray-100 items-start mb-4">
                    {[
                        { src: "/icons/Free_Order.webp", label: "Free Shipping Above ₹999", sizeClass: "w-28 h-28 md:w-32 md:h-32" },
                        { src: "/icons/Return.webp", label: "7 Days Replacement", sizeClass: "w-28 h-28 md:w-32 md:h-32" },
                        { src: "/icons/Free_delivery.webp", label: "Traditional Wood Pressed", sizeClass: "w-28 h-28 md:w-32 md:h-32" },
                        { src: "/icons/shield.webp", label: "Lab Tested Purity", sizeClass: "w-24 h-24 md:w-24 md:h-24" },
                    ].map((feature, idx) => (
